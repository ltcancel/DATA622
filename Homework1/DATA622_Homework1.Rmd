...
The Sales Channel and Order Priority variables are distinct categorical values and can be converted to factors so we will convert both dataframes. Both variables have a small number of possible values so we will use Order Priority for our Decision Tree.
...
```{r}
rtree2 <- rpart(Order.Priority ~ ., data = train_data2, method="class", control = rpart.control(minsplit = 20, minbucket = 7, maxdepth = 10, usesurrogate = 2, xval = 10))
rtree2
```
